-- Complimentary PRO grants (friends & family) — no Stripe payment required.

alter table public.user_entitlements
  add column if not exists complimentary_grant boolean not null default false;

comment on column public.user_entitlements.complimentary_grant is
  'Admin-granted PRO without purchase; revoked when grant is removed.';
...
comment on table public.admin_complimentary_grants is
  'Email allowlist for complimentary PRO; applied on signup or immediately if user exists.';
